...
- **HTTP(S) URL** - يتصل عبر نقطة نهاية JSON-RPC (على سبيل المثال، `https://mainnet.infura.io/v3/YOUR_KEY`).
...
| البيئة    | الخلفية          | ملاحظات                                              |
| --------- | ---------------- | ---------------------------------------------------- |
| نود.جي إس | سكليتي           | مخزنة في `dataPath`. يستخدم وضع WAL للوصول المتزامن. |
...
يمكن توصيل المحلل مباشرة بـ pkc-js من خلال خيار `nameResolvers`، مما يتيح تحليل اسم `.bso` الشفاف أثناء عمليات البحث عن المفاتيح:
...
تشحن الحزمة نقاط دخول منفصلة لإصدارات Node.js والمتصفح. ستقوم المجمعات التي تدعم حقل `exports` في `package.json` بتحديد الحقل الصحيح تلقائيًا.
