...
- **HTTP(S) URL** -- Menyambungkan melalui titik akhir JSON-RPC (misalnya, `https://mainnet.infura.io/v3/YOUR_KEY`).
...
| Peramban    | DB Terindeks       | Menggunakan transaksi IndexedDB asli.                               |
| Penggantian | `Map` dalam memori | Digunakan ketika SQLite atau IndexedDB tidak tersedia.              |
...
Resolver dapat dipasang langsung ke pkc-js melalui opsi `nameResolvers`, memungkinkan resolusi nama `.bso` transparan selama pencarian kunci:
...
Paket ini mengirimkan titik masuk terpisah untuk Node.js dan versi browser. Bundel yang mendukung bidang `exports` di `package.json` akan secara otomatis memilih yang benar.
