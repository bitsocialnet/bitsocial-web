...
A `plebbit-js` tetejére épül, és a [5chan](/apps/5chan/) és a [Seedit] (/apps/seedit/) használja közösség létrehozására és csomópontkezelésére.
...
| `create subplebbit` | Új közösség létrehozása                                   |
| `subplebbit edit`   | Közösségi beállítások frissítése (cím, leírás, szabályok) |
| `subplebbit list`   | Az ezen a csomóponton tárolt közösségek listázása         |
| `subplebbit start`  | Egy adott közösség kiszolgálásának megkezdése             |
| `subplebbit stop`   | Egy adott közösség kiszolgálásának leállítása             |

Futtassa bármelyik parancsot a `--help` segítségével az elérhető opciók és jelzők megtekintéséhez:
