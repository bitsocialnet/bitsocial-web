...
When React diagnostics would resolve a concern, use the installed version and its supported flags:

```bash
yarn doctor
```

For a regression check after making changes, run with `--scope changed` and resolve relevant newly introduced findings.
...
Run `yarn doctor` and resolve relevant newly introduced findings. Re-check every visual rubric row that failed, using the same viewpoint and interaction as the original evidence, and confirm it now passes. Then verify behavior: the scene renders, animations play, and interactions respond. If browser dev tools are available, watch the memory profile while orbiting an idle scene; a rising heap during idle means a disposal leak survived.
