export function assertHealthy(snapshot) {
  ...
  if (
    ...
    !snapshot.observedComponent ||
    (snapshot.profilerCommits.length && !snapshot.commits)
  )
    throw new Error("React profiling unavailable: Bippy has not observed component commits");
  if (!snapshot.profilerSupported)
    throw new Error(
      "React profiling unavailable: no React Profiler callback; use a development or profiling build with a Profiler boundary",
    );
  if (
    snapshot.events.some((event) => event.phase === "mount" || event.phase === "update") &&
    !snapshot.profilerCommits.length
  ) {
    throw new Error(
      "React profiling unavailable: component work has no timing callbacks in this measurement window",
    );
  }
...
export function summarize(snapshot, actionMs) {
  ...
  for (const commit of snapshot.profilerCommits) {
    if (!Number.isFinite(commit.actualDuration) || commit.actualDuration < 0)
      throw new Error("Invalid React Profiler duration");
  ...
  return {
    ...
    renderMs: Math.max(0, ...Object.values(boundaries)),
    boundaries,
    actionMs,
  };
}
...
export function checkBudgets(summary, budgets) {
  if (!budgets || Object.keys(budgets).length === 0)
    throw new Error("A checked phase needs an explicit budget");
  ...
  if (
    budgets.components !== undefined &&
    (!budgets.components ||
      typeof budgets.components !== "object" ||
      Array.isArray(budgets.components))
  )
  ...
  for (const [name, budget] of Object.entries(budgets.components || {})) {
    if (!budget || typeof budget !== "object" || !Object.keys(budget).length)
      throw new Error(`Empty component budget: ${name}`);
    for (const [key, limit] of Object.entries(budget)) {
      ...
      if (!Number.isInteger(limit) || limit < 0)
        throw new Error(`Invalid component budget: ${name}.${key}`);
      constraints += 1;
    }
  }
  ...
  for (const [name, budget] of Object.entries(budgets.components || {})) {
    const observed = summary.components[name] || {
      mounts: 0,
      updates: 0,
      unmounts: 0,
      typeIds: [],
    ...
    if (observed.typeIds.length > 1)
      failures.push(
        `${name}: ambiguous name identifies ${observed.typeIds.length} component types; inspect instance/type IDs and use a unique name for the budget`,
      );
    for (const phase of ["Updates", "Mounts", "Unmounts"]) {
      ...
      if (minimum !== undefined) {
        if (!Number.isFinite(minimum) || minimum < 0)
          throw new Error(`Invalid minimum for ${name}`);
        if (count < minimum)
          failures.push(`${name}.${phase.toLowerCase()}: ${count} is below ${minimum}`);
      }
    }
  }
  return failures;
