...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (name === "--headed") options.headed = true;
    ...
    ["--target", "--scenario", "--url", "--cpu", "--samples", "--output", "--actions"].includes(
      name,
    )
  ) {
...
if (!["check", "record", "selftest"].includes(command))
  throw new Error(`Unknown profiling command: ${command}`);
...
async function runCase(browser, target, scenario, origin, sample) {
  ...
  const context = await bounded(
    browser.newContext({ viewport: report.viewport }),
  ...
  const tracePath = path.join(
    output,
    `${safeName(target.name)}-${safeName(scenario.name)}-${sample}.json.gz`,
  );
  const measure = async (name, action, budgets, settings = {}) => {
      ...
      (names) =>
        new Promise((resolve, reject) => {
          ...
          const count = () => {
            ...
            return names.length
              ? data?.events.filter((event) => names.includes(event.name)).length
              : data?.commits;
          };
          let previous = count();
          const tick = () => {
            ...
            else if (now - started > 15000)
              reject(new Error("React did not become quiet before measurement"));
    ...
    if (navigated && !settings.navigation)
      throw new Error(
        `${name}: document changed; explicitly select navigation coverage to avoid losing the previous document's metrics`,
      );
    if (!navigated && data.epoch !== before.epoch)
  ...
  try {
    await Promise.race([
      ...
      (async () => {
        ...
        await page.goto(new URL(scenario.path || "/", origin).href, {
    ...
    if (
      result.crashed ||
      /target crashed|page crashed/i.test(error.message) ||
      !browser.isConnected()
    )
      abort.abort(error);
    try {
...
try {
  await withBrowser(packageRoot, !!options.headed, async (browser) => {
    ...
    let targets = config.targets.filter(
      (target) => !options.target || target.name === options.target,
    );
    if (!targets.length) throw new Error("No matching profiling target");
    if (options.url && targets.length !== 1)
      throw new Error("--url requires selecting one --target");
    let customScenario;
    if (options.actions)
      customScenario = (
        await import(pathToFileURL(path.resolve(process.cwd(), options.actions)).href)
      ).default;
    for (const target of targets) {
      const scenarios = customScenario
        ? [customScenario]
        : target.scenarios.filter(
      ...
      try {
        ...
        if (origin) {
          ...
          if (!["http:", "https:"].includes(url.protocol))
            throw new Error("--url must be an HTTP(S) app URL");
          ...
          server = await startServer(
            ...
            packageRoot,
            setup?.env || {},
            path.join(output, `${safeName(target.name)}-server.log`),
            abort.signal,
          );
        ...
        for (const scenario of scenarios) {
          for (let sample = 1; sample <= samples; sample += 1)
            await runCase(browser, target, scenario, origin, sample);
        }
      } finally {
        try {
          await server?.stop();
  ...
  report.ok =
    command === "selftest"
      ? !!report.selftest?.ok
      : report.cases.length > 0 && report.cases.every((result) => result.ok);
} catch (error) {
