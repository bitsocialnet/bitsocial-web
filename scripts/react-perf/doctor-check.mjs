...
if (!base) {
  const candidate = spawnSync("git", ["rev-parse", "--verify", "origin/master"], {
    cwd: root,
    encoding: "utf8",
  });
...
if (
  base &&
  spawnSync("git", ["rev-parse", "--verify", `${base}^{commit}`], { cwd: root, stdio: "ignore" })
    .status !== 0
)
...
if (base) {
  const revision = (ref) =>
    spawnSync("git", ["rev-parse", ref], { cwd: root, encoding: "utf8" }).stdout?.trim();
...
for (const project of config.doctorProjects || [{ cwd: "." }]) {
  ...
  const args = [
    path.resolve(path.dirname(cliPackage), bin),
    ".",
    "-y",
    "--verbose",
    "--blocking",
  ...
  console.log(
    `[doctor:check] ${cwd}; ${base ? `changes since ${base}` : "full source scan (no comparison base available)"}`,
  );
