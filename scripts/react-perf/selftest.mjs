...
export async function runSelftest(browser, packageRoot) {
  ...
  for (const variant of await resolveVariants(packageRoot)) {
    try {
      ...
      throw new Error(
        `React compatibility failed for ${variant.name} (${variant.reactVersion}): ${error.message}`,
        { cause: error },
      );
    }
  }
  return {
    ok: true,
    variants,
    checks: variants.flatMap((variant) =>
      variant.checks.map((check) => `${variant.name}: ${check}`),
    ),
  };
}
...
async function runVariant(browser, packageRoot, variant) {
  ...
  try {
    ...
    assert.deepEqual(
      [...new Set(renderers.map((renderer) => renderer.version))],
      [variant.reactVersion],
      `Fixture renderer must match ${variant.name}'s React version`,
    );
    assert.throws(
      () => assertHealthy({ ...initial, commits: 0, events: [] }),
      /Bippy has not observed/,
    );
    assert.ok(
      initial.events.some((event) => event.name === "StateProbe" && event.phase === "mount"),
    );
    assert.ok(
      initial.events.some((event) => event.name === "MemoProbe" && event.phase === "mount"),
    );
    ...
    assert.throws(
      () => checkBudgets(summarize(initial, 0), { components: { StateProbe: {} } }),
      /Empty component budget/,
    );
    ...
    const state = await measure(() =>
      page.getByRole("button", { name: "State 0", exact: true }).click(),
    );
    assert.equal(state.summary.components.StateProbe.updates, 1);
    checks.push("state update: one committed update under StrictMode");
    const props = await measure(() =>
      page.getByRole("button", { name: "Props 0", exact: true }).click(),
    );
    ...
    assert.ok(
      checkBudgets(props.summary, { components: { SameName: { maxUpdates: 10 } } }).some(
        (failure) => failure.includes("ambiguous"),
      ),
    );
    checks.push("prop updates, memo bailout, and distinct same-named component types");
    const contextUpdate = await measure(() =>
      page.getByRole("button", { name: "Context update", exact: true }).click(),
    );
    assert.equal(contextUpdate.summary.components.ContextProbe.updates, 1);
    checks.push("context update passes through memo");
    const first = await measure(() =>
      page.getByRole("button", { name: "Instance A 0", exact: true }).click(),
    );
    const second = await measure(() =>
      page.getByRole("button", { name: "Instance A 1", exact: true }).click(),
    );
    ...
    assert.equal(
      second.summary.instances.find((instance) => instance.name === "InstanceProbe").instanceId,
      a.instanceId,
    );
    assert.equal(
      initial.events.filter((event) => event.name === "InstanceProbe" && event.phase === "mount")
        .length,
      2,
    );
    checks.push("instance IDs remain stable across resets and distinguish siblings");
    const remount = await measure(() =>
      page.getByRole("button", { name: "Remount", exact: true }).click(),
    );
    ...
    const baseline = await measure(() =>
      page.getByRole("button", { name: "Regression 0/0", exact: true }).click(),
    );
    assert.deepEqual(
      checkBudgets(baseline.summary, {
        components: { RegressionProbe: { minUpdates: 1, maxUpdates: 1 } },
      }),
      [],
    );
    await load("/?regression=1");
    const regressed = await measure(() =>
      page.getByRole("button", { name: "Regression 0/0", exact: true }).click(),
    );
    assert.ok(
      checkBudgets(regressed.summary, { components: { RegressionProbe: { maxUpdates: 1 } } }).some(
        (failure) => failure.includes("RegressionProbe.updates"),
      ),
    );
