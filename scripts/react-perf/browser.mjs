...
export async function withBrowser(packageRoot, headed, run) {
  ...
  if (!channel && !existsSync(driver.chromium.executablePath()))
  ...
  try {
    ...
    const result = await exec(
      "bash",
      [wrapper, "open", session, "about:blank", `--config=${config}`],
    ...
    busy = error.code === 75;
    if (busy)
      error.message = `Browser slot busy; retry after the owning task closes its browser. ${error.stderr || ""}`;
    throw error;
  } finally {
    ...
    if (attempted && !busy) {
      try {
        const result = await exec("bash", [wrapper, "close", session], {
          cwd: repoRoot,
          env,
          timeout: 30000,
        });
    ...
    if (failures.length) {
      if (primaryError)
        process.stderr.write(
          `[perf] Cleanup failed: ${failures.map((error) => error.message).join("; ")}\n`,
        );
    ...
    try {
      ...
      while (true) {
        ...
        const result = await bounded(
          cdp.send("IO.read", { handle: stream }),
          "Trace read",
      ...
      try {
        if (stream)
          await bounded(cdp.send("IO.close", { handle: stream }), "Trace stream close", 2000);
      } finally {
        await bounded(cdp.detach(), "Trace detach", 2000);
      }
    }
...
export async function startServer(server, packageRoot, env, output, abortSignal) {
  ...
  const substitute = (value) =>
    String(value).replaceAll("{port}", String(port)).replaceAll("{origin}", origin);
  ...
  const child = spawn(command[0], command.slice(1), {
    ...
    env: {
      ...
      ...env,
      ...Object.fromEntries(
        Object.entries(server.env || {}).map(([key, value]) => [key, substitute(value)]),
      ),
    },
  ...
  const stop = async () => {
    if (child.pid) {
      ...
      kill("SIGTERM");
      for (let attempt = 0; groupAlive() && attempt < 30; attempt += 1)
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      try {
        const response = await fetch(new URL(server.readyPath || "/", origin), {
