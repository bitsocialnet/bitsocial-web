...
export function summarizePairs(runs) {
  ...
  for (const mode of ["deterministic", "jev"]) {
    const selected = [...pairs.values()].filter(
      (pair) => (pair.old || pair.combined)?.mode === mode,
    );
    ...
    summaries.push({
      ...
      combinedAdapterModes: [
        ...new Set(selected.map((pair) => pair.combined?.adapterMode).filter(Boolean)),
      ],
      ...
      medianPairedReductionFraction: median(
        comparable.map((pair) => 1 - pair.combined.lifecycleMs / pair.old.lifecycleMs),
      ),
...
export async function benchmarkRun(plan, { createDriver, runPlan, client, baseline }) {
  ...
  const usage = () => {
    ...
    return {
      ...difference,
      estimatedCostUsd: difference.usageMissing ? null : difference.knownCostSubtotalUsd,
    };
  };
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (!values.live) {
    console.log(
      JSON.stringify({
        offline: true,
        plans: plans.map(({ id, file }) => ({ id, file })),
        requests: 0,
      }),
    );
    return 0;
  ...
  const oldDriver = await import(
    pathToFileURL(path.join(baselineRoot, "scripts/jev/browser-playwright.mjs")).href
  );
  const oldPlan = await import(
    pathToFileURL(path.join(baselineRoot, "scripts/jev/browser-plan.mjs")).href
  );
  const cli = await findPlaywrightCli();
  process.env.PLAYWRIGHT_CLI_BIN = cli;
  const client = values.jev
    ? createJevClient({
        live: true,
        maxRequests,
        maxCostUsd,
  ...
  for (const entry of plans)
    for (let sample = -warmups; sample < samples; sample++) {
      ...
      for (const transport of order) {
        const result = await benchmarkRun(entry.plan, {
          ...variants[transport],
          client,
          baseline: !values.jev,
        });
