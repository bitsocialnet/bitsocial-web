...
function target(value) {
  if (
    ...
    !text(value.name) ||
    (value.nth !== undefined && (!Number.isInteger(value.nth) || value.nth < 0 || value.nth > 20))
  )
    fail("invalid_target");
  if (
    value.within !== undefined &&
    (!keys(value.within, ["role", "name"]) ||
      !roles.has(value.within.role) ||
      !text(value.within.name))
  )
    fail("invalid_target");
}
...
export function validatePlan(plan) {
  ...
  if (!local && (plan.allowRemote !== true || url.protocol !== "https:"))
    fail("remote_not_authorized");
  for (const flag of ["allowRemote", "allowSensitiveActions", "reloadBeforeFinal"])
    if (plan[flag] !== undefined && typeof plan[flag] !== "boolean") fail("invalid_plan");
  if (
  ...
  for (const action of plan.actions) {
    ...
    if (
      ["fill", "select"].includes(action.op)
        ? !text(action.value, 1000)
        : action.value !== undefined
    )
      fail("invalid_action");
    if (
      action.maxUses !== undefined &&
      (!Number.isInteger(action.maxUses) || action.maxUses < 1 || action.maxUses > 3)
    )
      fail("invalid_action");
    ...
    if (
      ...
      /\b(log ?in|sign ?in|sign ?up|post|publish|submit|delete|remove|pay|buy|purchase|transfer|send|password|secret|token)\b/i.test(
        action.name,
      )
    )
  ...
  if (
    ...
    plan.requiredActions.some((id) => !ids.has(id)) ||
    new Set(plan.requiredActions).size !== plan.requiredActions.length
  )
  ...
  for (const assertion of plan.assertions) {
    ...
    if (assertion.type === "url") {
      ...
      if (!/^[\w-]{1,100}$/.test(assertion.value) || typeof assertion.present !== "boolean")
        fail("invalid_assertion");
  ...
  if (
    plan.semanticChecks !== undefined &&
    (!Array.isArray(plan.semanticChecks) || plan.semanticChecks.length > 10)
  )
  ...
  return {
    ...
    assertions: plan.assertions.map((assertion) =>
      assertion.type === "url"
        ? { ...assertion, equals: new URL(assertion.equals).href }
        : assertion,
    ),
    limits,
  };
...
export function snapshotNodes(snapshot) {
  ...
  for (const line of snapshot.split("\n")) {
    ...
    if (node.role === "option" && stack.at(-1)?.role === "combobox")
      stack.at(-1).options.push(name);
    nodes.push(node);
    stack.push(node);
  }
  return nodes;
    ...
    const matches = nodes.filter(
      (node) =>
        ...
        (!action.within ||
          node.ancestors.some(
            (ancestor) =>
              ancestor.role === action.within.role && ancestor.name === action.within.name,
          )),
    );
    const node =
      action.nth === undefined ? (matches.length === 1 ? matches[0] : null) : matches[action.nth];
...
export async function runBrowserPlan(
  planInput,
  { driver, client, baseline = false, now = Date.now } = {},
) {
  ...
  async function observe(assertions) {
    ...
    if (
      ...
      Buffer.byteLength(observation.snapshot) > plan.limits.maxSnapshotBytes
    )
    ...
    if (assertions) {
      ...
      const results = Object.hasOwn(observation, "assertions")
        ? observation.assertions
        : await driver.assert(assertions);
      if (
        !Array.isArray(results) ||
        results.length !== assertions.length ||
        results.some((value) => typeof value !== "boolean")
      )
        fail("browser_assertions_invalid");
  ...
  try {
    ...
    for (let step = 0; step <= plan.limits.maxSteps; step++) {
      ...
      if (
        required &&
        report.assertions.length === plan.assertions.length &&
        report.assertions.every((a) => a === true)
      ) {
        if (plan.reloadBeforeFinal) {
          ...
          if (
            ...
            fail("persistence_assertion_failed");
        }
        withinDeadline();
        ...
        for (const check of plan.semanticChecks || []) {
          ...
          try {
            ...
            if (typeof content !== "string" || content.length > 8000)
              fail("semantic_text_unavailable");
            ...
            report.semantic.push({
              ...
              advisory: true,
              verdict:
                answer.probabilities[answer.choice] >= plan.limits.minProbability
                  ? answer.choice
                  : "uncertain",
            });
          } catch {
        ...
        if (report.semantic.length) {
          report.semanticStatus = report.semantic.every((check) => check.verdict === "satisfies")
            ? "satisfied"
            : baseline
              ? "not_run"
              : "review_required";
      ...
      if (!baseline) {
        const result = await client.ask({
          state: {
            goal: plan.goal,
            untrustedSnapshot: observation.snapshot,
            completedActionIds: history,
          },
          questions: {
            next_action: {
              type: "choice",
              ...
              criteria: Object.fromEntries([
                ...
                [
                  "hand_back",
                  "Return control because no offered action clearly advances the task.",
                ],
              ]),
            },
          },
        ...
        if (
          answer.choice === "hand_back" ||
          answer.probabilities[answer.choice] < plan.limits.minProbability
        )
          fail("model_uncertain");
  ...
  return {
    ...report,
    adapterMode: driver.adapterMode || "separate-driver",
