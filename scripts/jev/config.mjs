...
export function resolveJevSettings({ apiKey, model, env = process.env, home = os.homedir() } = {}) {
  ...
  if ((key === undefined && keyFile === undefined) || selectedModel === undefined) {
    ...
    const file = explicit
      ? env.JEV_CONFIG_FILE
      : path.join(env.XDG_CONFIG_HOME || path.join(home, ".config"), "bitsocial", "jev.json");
    ...
    if (raw !== undefined) {
      ...
      if (
        ...
        Object.keys(config).some((k) => !["apiKeyFile", "model"].includes(k))
      )
        fail("invalid_config");
    }
  }
  if (selectedModel === undefined) selectedModel = config.model;
  if (typeof selectedModel !== "string" || !/^jev-\d+\.\d+\.\d+$/.test(selectedModel))
    fail("pinned_model_required");
  if (key === undefined) {
...
export function redactJevSecrets(value) {
  ...
  const keys = [...new Set([...loadedKeys, process.env.TYPESAFE_API_KEY?.trim()])]
    .filter(Boolean)
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  if (process.argv.length !== 3 || process.argv[2] !== "--check") {
    console.log(
      "Usage: node scripts/jev/config.mjs --check (checks local credentials and pinned model; no network)",
    );
    ...
    try {
      ...
      console.log(
        JSON.stringify({
          ready: false,
          reason: error instanceof JevConfigError ? error.code : "configuration_unavailable",
          networkCalls: 0,
        }),
      );
      process.exitCode = 2;
