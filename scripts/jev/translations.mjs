...
export function parseScopedCsv(value, option) {
  ...
  if (!selected.length)
    throw new TranslationInputError(`${option} requires at least one nonempty selection`);
  return selected;
}
...
function positiveNumber(value, label, maximum = Infinity) {
  ...
  if (!Number.isFinite(number) || number <= 0 || number > maximum)
    throw new TranslationInputError(`Invalid ${label}`);
  return number;
}
...
export async function readTranslationJson(file, cwd) {
  ...
  if (stat.size > MAX_FILE_BYTES)
    throw new TranslationInputError(`JSON input exceeds 4 MiB: ${inputPath(file, cwd)}`);
  try {
...
export function flattenTranslations(value, prefix = "", result = Object.create(null)) {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new TranslationInputError("Locale JSON must be an object");
  for (const [key, entry] of Object.entries(value)) {
    const fullKey = prefix ? `${prefix}.${key}` : key;
    if (entry && typeof entry === "object" && !Array.isArray(entry))
      flattenTranslations(entry, fullKey, result);
    else {
      if (own(result, fullKey))
        throw new TranslationInputError("Ambiguous nested and dotted translation keys");
...
function balancedTags(text) {
  ...
  for (const match of text.matchAll(/<(\/?)([A-Za-z0-9][\w-]*)([^<>]*)>/g)) {
    ...
    } else if (!attributes.trimEnd().endsWith("/") && !voidTags.has(name.toLowerCase()))
      stack.push(name);
  }
...
export function structuralIssues(pair) {
  ...
  if (
    JSON.stringify(tokens(pair.source, placeholderPattern)) !==
    JSON.stringify(tokens(pair.translation, placeholderPattern))
  ) {
    issues.push("placeholder_mismatch");
  }
  if (JSON.stringify(tags(pair.source)) !== JSON.stringify(tags(pair.translation)))
    issues.push("tag_mismatch");
  ...
  if (
    JSON.stringify(protectedTokens(pair.source)) !==
    JSON.stringify(protectedTokens(pair.translation))
  )
    issues.push("protected_token_mismatch");
  return issues;
}
...
export function translationQuestions() {
  ...
  const criteria = {
    preserve:
      "The translation preserves this aspect of the source, or this aspect is absent in both.",
    issue:
      "The translation clearly changes, contradicts, invents, or loses this aspect of the source.",
    uncertain:
      "There is insufficient evidence or language understanding to establish preservation or a clear issue.",
  };
  return {
    meaning: {
      type: "choice",
...
function validAnswers(answers) {
  if (
    ...
    Object.keys(answers).length !== QUESTION_IDS.length
  )
    return false;
...
export function summarizeAnswers(answers) {
  ...
  if (
    QUESTION_IDS.every(
      (id) =>
        answers[id].choice === "preserve" && answers[id].probabilities.preserve >= SAFE_PROBABILITY,
    )
  )
    ...
    QUESTION_IDS.map((id) => [
      id,
      {
        ...
        probabilities: Object.fromEntries(
          CHOICES.map((choice) => [choice, answers[id].probabilities[choice]]),
        ),
      },
    ]),
  );
...
async function writeCache(directory, key, model, answers, now) {
  ...
  if (!directoryStat.isDirectory() || directoryStat.isSymbolicLink())
    throw new TranslationInputError("Unsafe cache directory");
  ...
  try {
    ...
    await handle.writeFile(
      `${JSON.stringify({ model, createdAt: now, answers: minimalAnswers(answers) })}\n`,
    );
  } finally {
    await handle.close();
  }
...
export async function reviewTranslations(
  pairs,
  { client, live = false, model, context = "", cacheDir, now = Date.now() } = {},
) {
  if (!pairs.length)
    throw new TranslationInputError("No translation pairs matched the requested scope");
  if (live && (!model || !/^jev-\d+\.\d+\.\d+$/.test(model)))
    throw new TranslationInputError(
      "Live translation QA requires an explicitly pinned JEV_MODEL or --model",
    );
  const results = [];
  ...
  return {
    ...
    summary: Object.fromEntries(
      ["pass", "flagged", "unverified"].map((status) => [
        status,
...
export async function loadLocalePairs({
  cwd = process.cwd(),
  translationsRoot = "public/translations",
  locales,
  keys = [],
  base,
  changedFiles = [],
  ...
  if (locales.some((locale) => !/^[A-Za-z0-9_-]+$/.test(locale) || locale === "en"))
    throw new TranslationInputError("Target locales must be locale codes other than en");
  if (!keys.length && !base && !changedFiles.length)
    throw new TranslationInputError(
      "Select --keys, --base, or --changed-files; implicit whole-catalog scans are disabled",
    );
  ...
  if (!Object.keys(source).length)
    throw new TranslationInputError("English source catalog is missing or empty");
  let repository;
  let revision;
  let changed;
      ...
      changedFiles.length
        ...
        : [
            ...git(repository, [
              ...
              revision,
              "--",
              gitRelativePath(repository, root),
            ]).split("\0"),
            ...git(repository, [
              "ls-files",
              "--others",
  ...
  const previous = (file) => {
    ...
    try {
      ...
      if (exists)
        throw new TranslationInputError(
          `Unable to parse locale JSON at requested Git base: ${inputPath(file, canonicalCwd)}`,
        );
  ...
  for (const locale of locales) {
    ...
    if (changed) {
      ...
      const filtered = new Set([
        ...englishKeys,
        ...(targetChanged ? changedKeys(previous(file), target) : []),
      ]);
      selected = new Set([...selected].filter((key) => filtered.has(key)));
    }
    for (const key of [...selected].sort())
      pairs.push({ key, locale, source: source[key], translation: target[key] });
  }
  if (!pairs.length)
    throw new TranslationInputError("No translation pairs matched the requested scope");
  return pairs;
...
export async function loadParagraphPairs(file, { locales = [], keys = [] } = {}) {
  ...
  if (!Array.isArray(pairs))
    throw new TranslationInputError('Pairs JSON requires an array or {"pairs": [...]}');
  const seen = new Set();
  if (pairs.some((pair) => !pair || typeof pair !== "object" || Array.isArray(pair)))
    throw new TranslationInputError("Each pair must be an object");
  const selected = pairs
    .filter(
      (pair) =>
        (!locales.length || locales.includes(pair.locale)) &&
        (!keys.length || keys.includes(pair.key)),
    )
    .map((pair) => {
      if (
        ...
        throw new TranslationInputError(
          "Each pair requires a key, locale, and optional string context/path",
        );
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const selection = {
    locales: parseScopedCsv(values.locales, "--locales"),
    keys: parseScopedCsv(values.keys, "--keys"),
  };
  const changedFiles = parseScopedCsv(values["changed-files"], "--changed-files");
  if (values.pairs && (values.base || values["changed-files"]))
    throw new TranslationInputError("--pairs cannot be combined with Git filtering");
  ...
  if (!Number.isInteger(maxPairs) || !Number.isInteger(maxRequests))
    throw new TranslationInputError("Pair/request limits must be integers");
  ...
  const pairs = values.pairs
    ...
    : await loadLocalePairs({
        ...selection,
        translationsRoot: values["translations-root"],
        base: values.base,
        changedFiles,
      });
  if (pairs.length > maxPairs)
    throw new TranslationInputError(
      `Selected ${pairs.length} pairs; narrow selection or explicitly increase --max-pairs (currently ${maxPairs})`,
    );
  ...
  const report = await reviewTranslations(pairs, {
    client,
    live: values.live,
    model,
