...
export function validateReviewInput(input, rules) {
  if (
    !keysOnly(input, ["version", "provenance", "items"]) ||
    input.version !== 1 ||
  ...
  if (
    ...
    !["synthetic", "human-reviewed", "unspecified"].includes(input.provenance)
  )
    fail("invalid_label_provenance");
  const seen = new Set();
  for (const item of input.items) {
    if (
      ...
      (item.context !== undefined &&
        (typeof item.context !== "string" || Buffer.byteLength(item.context) > 12_000))
    )
    ...
    if (
      ...
      (!object(item.expected) ||
        ...
        Object.entries(item.expected).some(
          ([id, value]) =>
            !rules.some((rule) => rule.id === id) ||
            !["clear", "issue", "uncertain"].includes(value),
        ))
    )
...
export function reviewQuestions(item, rules) {
  return Object.fromEntries(
    rules
      .filter(
        (rule) =>
          rule.extensions.includes(path.extname(item.path)) &&
          (!item.rules || item.rules.includes(rule.id)),
      )
      .map((rule) => [
        rule.id,
        {
          ...
          criteria: {
            clear:
              "The visible change complies with the rule, or the rule does not apply to the changed behavior.",
            issue:
              "The visible change introduces a concrete violation of this rule supported by the supplied evidence.",
            uncertain:
              "The available evidence is insufficient or ambiguous; a reviewer needs more context.",
          },
        },
      ]),
  );
  ...
  for (const item of items) {
    ...
    try {
      ...
      const checks = Object.entries(response.answers).map(([rule, answer]) => ({
        ...
        status:
          answer.choice === "uncertain" || answer.probabilities[answer.choice] < minProbability
            ? "unverified"
            : answer.choice,
      }));
      results.push({
        id: item.id,
        status: checks.some((x) => x.status === "issue")
          ? "issue"
          : checks.some((x) => x.status === "unverified")
            ? "unverified"
            : "clear",
        checks,
...
export function reviewMetrics(items, results) {
  const rows = items.flatMap((item) =>
    Object.entries(item.expected || {}).map(([rule, expected]) => ({
      ...
      expected,
      actual:
        results.find((r) => r.id === item.id)?.checks.find((c) => c.rule === rule)?.status ||
        "unverified",
    })),
  );
  ...
  return {
    ...
    unsupportedClears: rows.filter((x) => x.expected === "uncertain" && x.actual === "clear")
      .length,
    correctAbstentions: rows.filter((x) => x.expected === "uncertain" && x.actual === "unverified")
      .length,
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (
    Boolean(values.input) === Boolean(values.base) ||
    (values.input && values.files) ||
    (values.base && !values.files)
  )
    fail("explicit_review_scope_required");
  ...
  return report.items.some((x) => x.status === "issue")
    ? 1
    : report.items.some((x) => x.status === "unverified")
      ? 2
      : 0;
}
