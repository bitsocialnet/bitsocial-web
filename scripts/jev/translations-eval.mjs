...
export function evaluationMetrics(labels, results) {
  const byIdentity = new Map(
    results.map((result) => [JSON.stringify([result.locale, result.key]), result]),
  );
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (!pairs.length || pairs.length > 100)
    throw new Error("Evaluation requires 1 to 100 explicitly selected fixture pairs");
  const input = JSON.parse(await fs.readFile(values.corpus, "utf8"));
  const labels = (Array.isArray(input) ? input : input.pairs).filter(
    (pair) => !keys.length || keys.includes(pair.key),
  );
  if (labels.some((label) => !["pass", "flagged"].includes(label.expected)))
    throw new Error("Evaluation labels must be pass or flagged");
  if (keys.some((key) => !labels.some((label) => label.key === key)))
    throw new Error("Requested case is absent from corpus");
