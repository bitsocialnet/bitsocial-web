...
export function evaluationQuestions(rubric = "baseline") {
  if (!["baseline", "contrastive"].includes(rubric))
    throw new TranslationInputError("Rubric must be baseline or contrastive");
  const questions = translationQuestions();
  if (rubric === "baseline") return questions;
  const examples = {
    meaning: {
      preserve:
        "Example boundary: active and passive wording are equivalent when the same actor performs the same action.",
      issue:
        "Example boundary: permission to undo an operation becomes an obligation to undo it, or the actor receiving a notification becomes the sender.",
    },
    qualifications: {
      preserve:
        "Example boundary: a condition may move to another clause while retaining the same prerequisite and exception.",
      issue:
        "Example boundary: access while a session is open becomes permanent access; an action affecting one workspace is described as affecting every workspace.",
    },
    terminology: {
      preserve:
        "Example boundary: a conventional localized technical term denotes the same function even if its literal wording differs.",
      issue:
        "Example boundary: a recovery code is described as an account password, or exporting a copy is described as transferring ownership.",
    },
  };
...
export async function evaluateTranslationPairs(
  ...
  if (split !== "calibration" && thresholds.length)
    throw new TranslationInputError("Threshold sweeps are permitted only on the calibration split");
    ...
    evaluation: {
      ...
      semanticOnly:
        live && labeled.length
          ...
          : null,
      calibration:
        live && split === "calibration"
          ? calibrationReport(labels, report.results, observations, thresholds, split)
          : null,
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (
    values["expected-corpus-sha256"] !== undefined &&
    values["expected-corpus-sha256"] !== corpus.metadata.sha256
  )
  ...
  const thresholds =
    values.thresholds === undefined
      ? values.split === "calibration"
        ? [0.8, 0.9, 0.95, 0.99]
        : []
      : parseScopedCsv(values.thresholds, "--thresholds").map(Number);
  if (values.split !== "calibration" && thresholds.length)
    throw new TranslationInputError("Threshold sweeps are permitted only on the calibration split");
  ...
  console.log(
    JSON.stringify(
      { ...report, evaluation: { ...report.evaluation, corpus: corpus.metadata } },
      null,
      2,
    ),
  );
  ...
  return metrics?.missed || metrics?.falseAlarms
    ? 1
    : !metrics || metrics.unverified || report.evaluation.unlabeledCases
      ? 2
      : 0;
}
