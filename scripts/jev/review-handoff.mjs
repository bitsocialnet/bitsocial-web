...
export async function readReviewJson(file) {
  ...
  try {
    ...
    if (!info.isFile() || info.size > limit)
      fail("Review input must be a regular JSON file of at most 4 MiB");
...
export function createReviewQueue({ kind, corpusId, rubric, model = null, items }) {
  if (!["moderation", "translation"].includes(kind) || !token(corpusId))
    fail("Invalid review domain or corpus ID");
  if (
    !exact(rubric, ["version", "text", "sourceSha256"]) ||
    !token(rubric.version) ||
    !text(rubric.text, 30000) ||
    !sha(rubric.sourceSha256)
  )
    fail("Explicit rubric identity required");
  if (model !== null && !/^jev-\d+\.\d+\.\d+$/.test(model))
    fail("Model must be null (not run) or an explicit pinned Jev model");
  if (!Array.isArray(items) || !items.length || items.length > 100)
    fail("Select 1 to 100 explicit sanitized items");
  const seen = new Set(),
    splits = new Map(),
    provenance = new Set();
  const rows = items.map((item) => {
    ...
    if (
      ...
      (!text(source.translationPath, 300) ||
        source.translationPath.startsWith("/") ||
        source.translationPath.split("/").includes(".."))
    )
      fail("Invalid translation source path");
    if (source.key !== undefined && !text(source.key, 300)) fail("Invalid source key");
    if (source.index !== undefined && (!Number.isInteger(source.index) || source.index < 0))
    ...
    if (kind === "translation") {
      ...
      if (source.kind !== "synthetic")
        fail(
          "Moderation handoff currently accepts explicitly synthetic fixtures only; no private traffic importer",
        );
      if (
      ...
      if (
        ...
        ["title", "content", "link", "linkHtmlTagName"].some(
          (key) => p[key] !== undefined && (typeof p[key] !== "string" || p[key].length > 10000),
        ) ||
        (p.timestamp !== undefined && (!Number.isFinite(p.timestamp) || p.timestamp < 0)) ||
        (p.kind === "commentEdit" &&
          (typeof p.content !== "string" ||
            ["title", "link", "linkHtmlTagName"].some((key) => p[key] !== undefined))) ||
        (content.articleMaxAgeHours !== undefined &&
          (!Number.isFinite(content.articleMaxAgeHours) || content.articleMaxAgeHours <= 0))
      )
        fail("Invalid sanitized publication");
    }
    // This guard catches common accidental credentials; it is not a private-data sanitizer.
    if (
      /(?:\bsk-[A-Za-z0-9_-]{12,}|-----BEGIN [A-Z ]*PRIVATE KEY-----|(?:api[_ -]?key|authorization|password)\s*[:=]\s*\S{8,})/i.test(
    ...
    for (const key of [
      `group:${item.group}`,
      `input:${kind === "translation" ? reviewHash([content.locale, normalize(content.source)]) : contentSha256}`,
    ]) {
      if (splits.has(key) && splits.get(key) !== item.split)
        fail("Identical or related inputs cross frozen splits");
  ...
  const queue = {
    ...
    corpusId,
    model,
    rubric: { ...rubric, sha256: reviewHash(rubric) },
    items: rows,
  };
...
export function validateReviewQueue(queue) {
  ...
  if (
    queue.items.some(
      (item) => !exact(item, ["id", "contentSha256", "group", "split", "source", "content"]),
    )
  )
    fail("Unexpected review item fields");
  const { sha256: _rubricHash, ...rubric } = queue.rubric;
  const rebuilt = createReviewQueue({
    ...queue,
    rubric,
    items: queue.items.map(({ content, source, group, split }) => ({
      content,
      source,
      group,
      split,
...
export function blankReview(queue) {
  ...
  return {
    ...
    labels: queue.items.map(({ id, contentSha256 }) => ({
      id,
      contentSha256,
      label: "",
      notes: "",
...
export function importReview(queue, review) {
  ...
  if (
    ...
    fail(
      "Review identity, valid date, human reviewer, independence attestation, and every item are required",
    );
  ...
  const rows = queue.items.map((item) => {
    ...
    if (!row || row.contentSha256 !== item.contentSha256)
      fail("Unknown item or changed content identity");
    return {
      ...item,
      label: row.label,
      notes: row.notes,
      state: allowed.includes(row.label) ? "labeled" : "pending",
    };
  });
  return {
    schemaVersion: 1,
    ...
    summary: {
      total: rows.length,
      labeled: rows.filter((row) => row.state === "labeled").length,
...
export function selectReview(queue, input) {
  ...
  if (
    !exact(input, ["schemaVersion", "reviews"]) ||
    input.schemaVersion !== 1 ||
    !Array.isArray(input.reviews) ||
    !input.reviews.length ||
    input.reviews.length > 10
  )
...
export function reviewHtml(queues) {
  ...
  if (
    ...
    new Set(queues.flatMap((q) => q.items.map((item) => item.id))).size !==
      queues.reduce((count, q) => count + q.items.length, 0)
  )
