...
export function validateQuestions(questions) {
  if (!object(questions) || Object.keys(questions).length < 1 || Object.keys(questions).length > 20)
    ...
    if (
      ...
      choices.some(
        (key) =>
          ...
          typeof question.criteria[key] !== "string" ||
          question.criteria[key].length > 2000,
      )
    )
      fail("invalid_questions");
  }
  ...
  for (const [id, question] of Object.entries(questions)) {
    ...
    answers[id] = {
      ...
      probabilities: Object.fromEntries(
        choices.map((choice) => [choice, answer.probabilities[choice]]),
      ),
    };
  }
  const usage = {};
...
export function createJevClient({
  ...
  if (
    ![maxCalls, maxInputBytes, maxInputTokens, timeoutMs, deadlineMs].every(
  ...
  async function ask({ state, questions }) {
    ...
    try {
      ...
      if (!response.ok)
        fail(response.status === 429 ? "provider_throttled" : `provider_http_${response.status}`);
