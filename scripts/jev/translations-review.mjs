...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (!values.queue || !values.review || !values.out)
    throw new Error("Explicit queue, review bundle, and new output prefix required");
  const result = importTranslationReview(
    await readReviewJson(values.queue),
    await readReviewJson(values.review),
  );
  for (const [name, value] of Object.entries(result))
    if (value !== null) await writeReviewJson(`${values.out}.${name}.json`, value);
  console.log(
    JSON.stringify({
      outputPrefix: values.out,
