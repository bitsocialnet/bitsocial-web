    ...
    ["meaning", "qualifications", "terminology"].map((id) => [
      id,
      {
        ...
        probabilities: Object.fromEntries(
          ["preserve", "issue", "uncertain"].map((item) => [
            item,
            item === choice ? confidence : (1 - confidence) / 2,
          ]),
        ),
      },
    ]),
...
test("translation and evaluation resolve the machine model before reporting or caching live decisions", async (t) => {
  ...
  const configure = (selectedModel) =>
    fs.writeFile(configFile, JSON.stringify({ apiKeyFile, model: selectedModel }), { mode: 0o600 });
    ...
    return new Response(
      JSON.stringify({
        ...
        answers: Object.fromEntries(
          Object.entries(answers()).map(([id, answer]) => [id, { ...answer, type: "choice" }]),
        ),
  ...
  assert.deepEqual(
    ...
    [translationCacheKey(goodPair, model), translationCacheKey(goodPair, "jev-1.14.0")]
      .map((key) => `${key}.json`)
      .sort(),
  );
...
test("placeholder-preserving reversed meaning is structurally valid and requires semantic QA", () => {
  assert.deepEqual(
    structuralIssues({
      source: "You cannot delete {{count}} posts.",
      translation: "Vous pouvez supprimer {{count}} publications.",
    }),
    [],
  );
});
...
test("structural checks catch missing keys, placeholder multiplicity, tags, URLs, addresses and code", () => {
  ...
  assert.deepEqual(
    structuralIssues({ source: "{{count}} + {{count}}", translation: "{{count}}" }),
    ["placeholder_mismatch"],
  );
  assert.deepEqual(structuralIssues({ source: "<0>One</0>", translation: "<1>Uno</1>" }), [
    "tag_mismatch",
  ]);
  assert.deepEqual(
    structuralIssues({
      source: "Open https://example.org.",
      translation: "Apri https://other.org.",
    }),
    ["protected_token_mismatch"],
  );
  assert.deepEqual(
    structuralIssues({ source: "Run `export-key`.", translation: "Esegui `import-key`." }),
    ["protected_token_mismatch"],
  );
  assert.deepEqual(
    structuralIssues({
      source: "0x0000000000000000000000000000000000000001",
      translation: "0x0000000000000000000000000000000000000002",
    }),
    ["protected_token_mismatch"],
  );
});
...
test("natural reordering keeps structural checks green", () => {
  assert.deepEqual(
    structuralIssues({
      source: "<b>{{name}}</b> has {{count}} posts.",
      translation: "{{count}} pubblicazioni di <b>{{name}}</b>.",
    }),
    [],
  );
});
...
test("same tag inventory with broken nesting is still a structural failure", () => {
  assert.deepEqual(
    structuralIssues({ source: "<b><i>One</i></b>", translation: "<b><i>Uno</b></i>" }),
    ["unbalanced_tags"],
  );
});
...
test("nested keys flatten while ambiguous dotted keys fail", () => {
  ...
  assert.throws(
    () => flattenTranslations({ nav: { help: "Help" }, "nav.help": "Other" }),
    /Ambiguous/,
  );
});
...
test("offline and structural failures never call the provider", async () => {
  ...
  const report = await reviewTranslations(
    [goodPair, { ...goodPair, key: "missing", translation: undefined }],
    { client },
  );
...
test("malformed, missing, extra and contradictory probability answers never pass", () => {
  for (const value of [
    ...
    {
      ...answers(),
      meaning: { ...answers().meaning, probabilities: { preserve: 1, issue: 1, uncertain: 1 } },
    },
  ]) {
...
test("only explicitly selected pair text and context are submitted, without labels or paths", async () => {
  ...
  const report = await reviewTranslations([pair], {
    client,
    live: true,
    model,
    context: "Cryptographic keys are not passwords.",
  });
  assert.equal(report.results[0].status, "pass");
  assert.equal(report.results[0].path, pair.path);
  assert.deepEqual(Object.keys(client.requests[0].questions), [
    "meaning",
    "qualifications",
    "terminology",
  ]);
...
test("omitted filters allow explicit pair-file scope, but provided blank CSV filters never widen it", () => {
  ...
  for (const value of ["", " ", ",", " , , "])
    assert.throws(() => parseScopedCsv(value, "--keys"), /--keys requires/);
});
    ...
    for (const value of ["", " ", ",", " , , "]) {
      await assert.rejects(
        () =>
          translationMain([
            "--pairs",
            "/nonexistent-unread-fixture.json",
            "--live",
            "--model",
            model,
            option,
...
test("model failures and mismatched model identities stay unverified without echoed secrets", async () => {
  ...
  await assert.rejects(
    () => reviewTranslations([goodPair], { live: true, model: "jev-latest" }),
    /pinned/,
  );
});
...
test("cache is private and contains no source, translation, key, domain context, or API key", async (t) => {
  ...
  const client = fakeClient();
  await reviewTranslations([goodPair], {
    client,
    live: true,
    model,
    cacheDir,
  ...
  for (const sentinel of [goodPair.source, goodPair.translation, goodPair.key, "context-sentinel"])
    assert.ok(!contents.includes(sentinel));
  assert.equal((await fs.stat(cacheDir)).mode & 0o777, 0o700);
  assert.equal((await fs.stat(path.join(cacheDir, file))).mode & 0o777, 0o600);
  const cached = await reviewTranslations([goodPair], {
    client,
    live: true,
    model,
    cacheDir,
...
test("shared client enforces a real request limit and preserves unverified budget results", async () => {
    ...
    fetchImpl: async () => {
      ...
      return new Response(
        JSON.stringify({
          model,
          answers: Object.fromEntries(
            Object.entries(answers()).map(([id, value]) => [id, { ...value, type: "choice" }]),
          ),
          usage: { input_tokens: 500, output_tokens: 12 },
        }),
      );
  ...
  const report = await reviewTranslations([goodPair, { ...goodPair, key: "next" }], {
    client,
    live: true,
    model,
  });
...
test("locale scans require explicit locales and keys or Git filtering", async (t) => {
  ...
  await assert.rejects(
    () => loadLocalePairs({ cwd, locales: ["../secrets"], keys: ["a"] }),
    /locale codes/,
  );
});
...
test("Git changed-file paths match English and target catalogs on Windows and POSIX", () => {
  ...
  for (const pathApi of [path.win32, path.posix]) {
    ...
    for (const locale of ["en", "it"]) {
      ...
      assert.ok(
        changed.has(gitRelativePath(repository, file, pathApi)),
        `${pathApi.sep} ${locale}`,
      );
    }
    assert.ok(
      !changed.has(
        gitRelativePath(
          repository,
          pathApi.join(repository, "public/translations/fr/default.json"),
          pathApi,
        ),
      ),
    );
...
test("both CLIs report bounded malformed pair or corpus paths and redact the API token", async (t) => {
  ...
  for (const [script, option] of [
    ...
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL(`../${script}`, import.meta.url)), option, relative],
      {
        cwd,
        encoding: "utf8",
    ...
    assert.match(
      result.stderr,
      /Unable to read valid JSON input: \.\.\..*\[redacted\]\/broken\.json/,
    );
...
test("Git base includes only changed keys plus English changes across selected locales", async (t) => {
  ...
  await json(path.join(cwd, "public/translations/en/default.json"), {
    a: "Changed",
    b: "Two",
    c: "Three",
  });
  await json(path.join(cwd, "public/translations/it/default.json"), {
    a: "Uno",
    b: "DUE",
    c: "Tre",
  });
  ...
  await json(path.join(cwd, "public/translations/es/default.json"), {
    a: "Uno",
    b: "Dos",
    c: "Tres",
  });
...
test("paragraph input supports paths and excludes labels/unrelated text", async (t) => {
  ...
  await json(file, [
    { ...goodPair, expected: "pass", path: "docs/it/keys.md", private: "exclude-me" },
  ]);
...
test("zero selected pairs is unverified rather than successful empty coverage", async (t) => {
  ...
  await assert.rejects(
    () => loadLocalePairs({ cwd, locales: ["it"], base: "HEAD" }),
    /No translation pairs/,
  );
});
