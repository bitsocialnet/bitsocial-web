...
function cleanEnv(home) {
  ...
  for (const key of ["TYPESAFE_API_KEY", "TYPESAFE_API_KEY_FILE", "JEV_MODEL", "JEV_CONFIG_FILE"])
    delete env[key];
  return env;
}
...
test("one machine configuration supplies every checkout and redacts the file-backed key", (t) => {
  ...
  assert.deepEqual(resolveJevSettings({ env: {}, home }), {
    apiKey: "file-fixture-key",
    model: "jev-1.13.0",
  });
...
test("redaction removes longer credentials before any previously loaded prefix", () => {
  ...
  assert.equal(
    redactJevSecrets("path/prefix-fixture-private-suffix/plan.json prefix-fixture"),
    "path/[redacted]/plan.json [redacted]",
  );
});
...
test("explicit options then environment override local defaults; complete overrides ignore broken config", (t) => {
  ...
  const env = {
    ...
    JEV_MODEL: "jev-2.0.0",
    JEV_CONFIG_FILE: "/nonexistent",
  };
  assert.deepEqual(resolveJevSettings({ env, home }), {
    apiKey: "environment-key",
    model: "jev-2.0.0",
  });
  assert.deepEqual(resolveJevSettings({ env, home, apiKey: "explicit-key", model: "jev-3.0.0" }), {
    apiKey: "explicit-key",
    model: "jev-3.0.0",
  });
  writeFileSync(configFile, "{invalid");
  assert.deepEqual(
    resolveJevSettings({ env: { TYPESAFE_API_KEY_FILE: keyFile, JEV_MODEL: "jev-2.0.0" }, home }),
    { apiKey: "file-fixture-key", model: "jev-2.0.0" },
  );
});
...
test("XDG and explicit config paths are honored without repository-relative credential discovery", (t) => {
  ...
  assert.equal(
    resolveJevSettings({ env: { JEV_CONFIG_FILE: configFile }, home: "/not-used" }).model,
    result.model,
  );
  assert.throws(() => resolveJevSettings({ env: { JEV_CONFIG_FILE: "relative.json" }, home }), {
...
test("malformed, oversized, unreadable and unpinned config errors disclose no content", (t) => {
  ...
  for (const value of [
    "{private-content",
    "x".repeat(16_385),
    JSON.stringify({ apiKey: "private-content" }),
    "[]",
  ]) {
    ...
    assert.throws(
      ...
      (error) =>
        ...
        ["invalid_config", "config_unreadable"].includes(error.code),
    );
  }
  assert.throws(
    () => resolveJevSettings({ env: { JEV_CONFIG_FILE: path.join(home, "absent") }, home }),
    { code: "config_unreadable" },
  );
  assert.throws(
    () => resolveJevSettings({ apiKey: "fixture", model: "jev-latest", env: {}, home }),
    { code: "pinned_model_required" },
  );
  assert.throws(
    () =>
      resolveJevSettings({
        env: { TYPESAFE_API_KEY_FILE: "relative.key", JEV_MODEL: "jev-1.13.0" },
        home,
      }),
  ...
  for (const content of ["", "key\nsecond-line", "x".repeat(16_385)]) {
    ...
    assert.throws(() =>
      resolveJevSettings({
        env: { TYPESAFE_API_KEY_FILE: keyFile, JEV_MODEL: "jev-1.13.0" },
        home,
      }),
    );
  }
...
test("CLI check is safe; a live client uses file credentials and fixed model without exporting secrets", (t) => {
  ...
  const check = spawnSync(process.execPath, [configPath, "--check"], {
    env,
    cwd: tmpdir(),
    encoding: "utf8",
  });
