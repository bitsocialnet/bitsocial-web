...
test("trial sends its exact questions, withholds labels and never reuses another rubric response", async () => {
  ...
  for (const rubric of ["baseline", "contrastive"]) {
    const report = await evaluateTranslationPairs([pair], [label], {
      client,
      live: true,
      model: "jev-1.13.0",
      rubric,
  ...
  await assert.rejects(
    () => evaluateTranslationPairs([pair], [label], { client, live: true, rubric: "unknown" }),
    /Rubric/,
  );
  assert.equal(requests.length, 2);
});
