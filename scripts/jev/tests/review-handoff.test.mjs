...
const item = (changes = {}) => ({
  ...
  content: {
    locale: "it",
    source: "Do not share your password.",
    translation: "Non condividere la tua password.",
  },
  group: "group-one",
  split: "calibration",
  ...changes,
...
test("queue and blank export contain no prior expectations or provider answers", () => {
  ...
  assert.throws(
    () => create([item({ content: { ...item().content, expected: "pass", answers: {} } })]),
    /sanitized/,
  );
...
test("related groups remain in one split, duplicate items rejected", () => {
  ...
  assert.throws(
    () =>
      create([
        item(),
        item({ content: { ...item().content, source: "Other source" }, split: "holdout" }),
      ]),
    /cross frozen splits/,
  );
});
...
test("missing, unknown, duplicate, and conflicting labels do not silently pass", () => {
  ...
  assert.throws(
    () =>
      selectReview(queue, {
        schemaVersion: 1,
        reviews: [reviewed(queue), reviewed(queue, "flagged")],
      }),
    /conflicting/,
  );
});
...
test("HTML is offline, escaped, syntactically valid, and has no model label leak", () => {
  const queue = create([
    item({ content: { ...item().content, translation: "</script><img src=x onerror=alert(1)>" } }),
  ]);
...
test("obvious secrets and unbounded inputs are rejected without reading credentials", () => {
  assert.throws(
    () => create([item({ content: { ...item().content, source: "api_key=abcdefghijklmno" } })]),
    /secret/,
  );
...
test("rendered page exports importer-compatible answers and safely restores a draft", async () => {
  ...
  const ids = Object.fromEntries(
    ["items", "reviewer", "date", "independent", "download", "message", "resume"].map((id) => [
      id,
      new Element(id),
    ]),
  );
  ...
  ids.reviewer.value = "";
  await ids.resume.listeners.change({
    target: { files: [{ size: 2000, text: async () => JSON.stringify(draft) }] },
  });
