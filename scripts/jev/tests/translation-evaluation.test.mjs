...
const corpus = (pairs) => ({
  schemaVersion: 1,
  corpusId: "fixture-v1",
  provenance: { kind: "synthetic", authoredBy: "model" },
  pairs,
});
  ...
  Object.fromEntries(
    ["meaning", "qualifications", "terminology"].map((id) => [
      id,
      {
        ...
        probabilities:
          choice === "issue"
            ? { preserve: 1 - p, issue: p, uncertain: 0 }
            : { preserve: p, issue: 1 - p, uncertain: 0 },
      },
    ]),
  );
const result = (entry, status) => ({
  ...
  status,
  origin: "provider",
  issues: status === "flagged" ? ["semantic_meaning"] : [],
});
...
test("versioned corpora require provenance, per-human label reviews and consistent split families", () => {
  ...
  assert.throws(
    () =>
      validateEvaluationCorpus(
        corpus([label("a"), { ...label("b", "pass", "holdout"), group: "a" }]),
      ),
    /leaks/,
  );
  assert.throws(
    () =>
      validateEvaluationCorpus(
        corpus([label("a"), { ...label("b", "pass", "holdout"), source: "Source   a " }]),
      ),
    /leaks/,
  );
...
test("selection excludes holdout by default and strips expected/review/split metadata from model pairs", () => {
  ...
  assert.deepEqual(Object.keys(selected.pairs[0]).sort(), [
    "context",
    "key",
    "locale",
    "source",
...
test("risk denominators include abstentions and distinguish coverage from empirical false-pass risk", () => {
  ...
  const metrics = evaluationMetrics(labels, [
    result(labels[0], "pass"),
    result(labels[1], "unverified"),
    result(labels[2], "pass"),
    result(labels[3], "flagged"),
  ]);
...
test("threshold sweeps only alter semantic passes and never convert unavailable or malformed evidence to passes", () => {
  ...
  const results = [
    result(labels[0], "unverified"),
    result(labels[1], "flagged"),
    { ...result(labels[2], "unverified"), origin: "none" },
    result(labels[3], "pass"),
  ];
  ...
  assert.throws(
    () => calibrationReport(labels, results, observations, [0.9], "holdout"),
    /calibration/,
  );
...
test("audit queue uses uncertain plus reproducible random complement and never selects holdout", () => {
  ...
  const queue = selectAuditExamples(labels, results, observations, {
    uncertain: 1,
    random: 2,
    seed: "example",
  });
  ...
  assert.deepEqual(
    queue,
    selectAuditExamples([...labels].reverse(), results, observations, {
      uncertain: 1,
      random: 2,
      seed: "example",
    }),
  ...
  assert.throws(
    () => selectAuditExamples(labels, results, observations, { random: -1 }),
    /sample sizes/,
  );
});
...
test("fresh evaluation captures probabilities with labels withheld, includes structural results and never retries failure", async () => {
  ...
  const client = {
    ask: async ({ state }) => {
      ...
      return {
        model: "jev-1.13.0",
        answers: answers(
          state.source === "Source a" ? 0.92 : 0.97,
          state.source === "Source issue" ? "issue" : "preserve",
        ),
      };
    },
    stats: () => ({ requests }),
  ...
  assert.equal(
    report.evaluation.calibration.thresholds.find((entry) => entry.threshold === 0.9).passCoverage,
    1 / 3,
  );
...
test("offline evaluation never calls provider and invalid options fail before calls", async () => {
  ...
  await assert.rejects(
    () => evaluateTranslationPairs(pairs, labels, { client, live: true, thresholds: [NaN] }),
    /threshold/,
  );
  await assert.rejects(
    () => evaluateTranslationPairs(pairs, labels, { client, live: true, split: "holdout" }),
    /calibration/,
  );
...
test("CLI guards fixed corpus hash and holdout tuning before credential discovery", async (t) => {
  ...
  for (const [args, expected] of [
    ...
    assert.throws(
      () =>
        execFileSync(process.execPath, [cli, "--corpus", file, "--live", ...args], {
          env,
          stdio: "pipe",
        }),
      (error) => {
...
test("shipped synthetic corpus has fixed separated contrast groups, no human gold claim", async () => {
  const input = JSON.parse(
    await fs.readFile(new URL("../fixtures/translations.json", import.meta.url), "utf8"),
  );
...
test("unlabeled audit examples can be selected for human review but never count as correctness evidence", async () => {
  ...
  assert.throws(
    () => validateEvaluationCorpus(corpus([{ ...unlabeled, split: "calibration" }])),
    /require expected/,
  );
