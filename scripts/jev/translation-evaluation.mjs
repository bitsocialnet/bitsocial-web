...
export function validateEvaluationCorpus(input) {
  ...
  if (!Array.isArray(pairs) || !pairs.length || pairs.length > 5000)
    failure("Evaluation corpus requires 1 to 5000 pairs");
  ...
  if (
    !legacy &&
    (!text(input.corpusId) ||
      !["synthetic", "independently-reviewed", "unreviewed"].includes(provenance?.kind) ||
      !text(provenance.authoredBy))
  )
    failure("Versioned corpus requires corpusId and explicit provenance");
  const seen = new Set();
  const assignments = new Map();
  const normalized = pairs.map((pair) => {
    if (!pair || !text(pair.key) || !text(pair.locale) || !text(pair.category))
      failure("Every evaluation pair requires locale, key, and category");
    ...
    if (!["pass", "flagged"].includes(pair.expected) && !unlabeledAudit)
      failure(
        "Calibration and holdout require expected pass/flagged labels; only audit may be unlabeled",
      );
    if (
    ...
    if (!splits.includes(split) || !text(group))
      failure("Every versioned pair requires a split and contrast group");
    if (
      provenance.kind === "independently-reviewed" &&
      (unlabeledAudit || !text(pair.review?.reviewer) || pair.review?.labelSource !== "human")
    )
    ...
    for (const family of [
      ...
      if (assignments.has(family) && assignments.get(family) !== split)
        failure("Evaluation group or source leaks across splits");
  ...
  return {
    ...
    metadata: {
      ...
      splits: Object.fromEntries(
        splits.map((split) => [split, normalized.filter((pair) => pair.split === split).length]),
      ),
...
export function selectEvaluationPairs(corpus, { split = "calibration", keys = [] } = {}) {
  ...
  const labels = corpus.pairs.filter(
    (pair) => pair.split === split && (!keys.length || keys.includes(pair.key)),
  );
  if (keys.some((key) => !labels.some((pair) => pair.key === key)))
    failure("Requested case is absent from the selected split");
  if (!labels.length || labels.length > 100)
    failure("Select 1 to 100 evaluation pairs from one split");
  const pairs = labels.map(({ key, locale, source, translation, context = "", path }) => ({
    key,
    locale,
    source,
    translation,
...
export function riskUpperBound(errors, count) {
  ...
  return Math.min(
    1,
    (p + z2 / (2 * count) + 1.96 * Math.sqrt((p * (1 - p) + z2 / (4 * count)) / count)) /
      (1 + z2 / count),
  );
}
...
export function evaluationMetrics(labels, results) {
  ...
  const cases = labels.map((label) => ({
    ...
    actual: ["pass", "flagged"].includes(byIdentity.get(identity(label))?.status)
      ? byIdentity.get(identity(label)).status
      : "unverified",
  }));
...
export function thresholdResults(results, observations, threshold) {
  if (!Number.isFinite(threshold) || threshold <= 0.5 || threshold > 1)
    failure("Pass threshold must be greater than 0.5 and at most 1");
  return results.map((result) => {
    ...
    if (score === null)
      return { ...result, status: "unverified", issues: ["invalid_provider_answers"] };
    const summary = summarizeAnswers(answers);
    if (summary.status === "flagged") return result;
    const passes =
      Object.values(answers).every((answer) => answer.choice === "preserve") && score >= threshold;
    return {
      ...result,
      status: passes ? "pass" : "unverified",
      issues: passes ? [] : ["semantic_uncertainty"],
    };
  });
}
...
export function calibrationReport(labels, results, observations, thresholds, split) {
  if (split !== "calibration")
    failure("Threshold sweeps are permitted only on the calibration split");
  ...
  return {
    ...
    thresholds: thresholds.map((threshold) => ({
      threshold,
      ...evaluationMetrics(semantic, thresholdResults(results, observations, threshold)),
    })),
    scoreBands: [
      [0, 0.5],
      [0.5, 0.8],
      ...
      return {
        ...
        high,
        cases: bucket.length,
        observedIssueFraction: ratio(
          bucket.filter((label) => label.expected === "flagged").length,
          bucket.length,
        ),
      };
    }),
...
export function selectAuditExamples(
  labels,
  results,
  observations,
  { uncertain = 5, random = 5, seed = "translation-audit-v1" } = {},
) {
  if (
    ![uncertain, random].every((value) => Number.isInteger(value) && value >= 0 && value <= 100) ||
    !text(seed) ||
    seed.length > 128
  )
    failure(
      "Audit sample sizes must be integers from 0 to 100 and seed must be 1 to 128 characters",
    );
  const byIdentity = new Map(results.map((result) => [identity(result), result]));
  const eligible = labels.filter((label) => label.split !== "holdout");
  const hashOrder = (a, b) =>
    digest([seed, identity(a)]).localeCompare(digest([seed, identity(b)]));
  const uncertainEntries = eligible
    .filter((label) => (byIdentity.get(identity(label))?.status || "unverified") === "unverified")
    .sort(
      (a, b) =>
        (observationScore(observations.get(identity(a))) ?? -1) -
          (observationScore(observations.get(identity(b))) ?? -1) || hashOrder(a, b),
    )
    .slice(0, uncertain);
  ...
  const entry = (label, reason) => ({
    ...
    locale: label.locale,
    reason,
    status: byIdentity.get(identity(label))?.status || "unverified",
  });
  return {
    seed,
    ...
    examples: [
      ...uncertainEntries.map((label) => entry(label, "uncertain")),
      ...randomEntries.map((label) => entry(label, "random")),
    ],
  };
}
