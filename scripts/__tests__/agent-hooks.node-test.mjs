...
test("normalizes Claude and Cursor edits, including paths with shell metacharacters", () => {
  ...
  assert.deepEqual(
    editedPaths({ hook_event_name: "afterFileEdit", file_path: "src/a $(touch sentinel).tsx" }),
    ["src/a $(touch sentinel).tsx"],
  );
});
...
*** End Patch`;
  assert.deepEqual(editedPaths({ tool_name: "apply_patch", tool_input: { command } }), [
    "src/new.ts",
    "src/update.ts",
    "src/renamed.ts",
  ]);
...
test("read-only, failed, malformed, and irrelevant invocations do no work", (t) => {
  ...
  for (const payload of payloads) {
    formatEditedFiles(payload, root, () =>
      assert.fail(`Unexpected formatter invocation: ${JSON.stringify(payload)}`),
    );
  }
});
...
test("missing formatter dependencies skip work without installing anything", (t) => {
  const { root, write } = fixture(t);
  write("file.ts");
  formatEditedFiles({ file_path: "file.ts" }, root, () =>
    assert.fail("Must not bootstrap dependencies"),
  );
});
