...
export function editedPaths(payload) {
  ...
  if (
    ...
    !["PostToolUse", "afterFileEdit"].includes(payload.hook_event_name)
  )
    return [];
  if (payload.tool_name && !editTools.has(payload.tool_name)) return [];
  if (payload.tool_response?.isError === true || payload.tool_response?.success === false)
    return [];
...
export function formattingPaths(payload, root = repoRoot) {
  ...
  const editCwd =
    typeof payload?.cwd === "string" && path.isAbsolute(payload.cwd) ? payload.cwd : root;
  const files = new Set();
  for (const candidate of editedPaths(payload)) {
    if (!candidate || candidate.includes("\0") || !/\.(?:[cm]?js|jsx|[cm]?ts|tsx)$/.test(candidate))
      continue;
...
export function formatEditedFiles(payload, root = repoRoot, run) {
  ...
  if (result.error || result.status !== 0) {
    console.error(
      `[agent format] Formatting failed: ${result.error?.message || result.stderr?.trim() || `exit ${result.status}`}`,
    );
  }
}
