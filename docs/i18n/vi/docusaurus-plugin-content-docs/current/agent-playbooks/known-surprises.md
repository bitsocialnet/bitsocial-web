...
- **Điều đáng ngạc nhiên:** URL cục bộ mặc định không phải là cổng Vite thông thường. Kho lưu trữ mong đợi `http://bitsocial.localhost:1355` thông qua Portless, vì vậy việc kiểm tra `localhost:3000` hoặc `localhost:5173` có thể nhấn nhầm ứng dụng hoặc không có gì cả.
...
- **Giảm nhẹ:** Sử dụng `http://bitsocial.localhost:1355` trước. Chỉ bỏ qua nó với `PORTLESS=0 corepack yarn start` khi bạn thực sự cần một cổng Vite trực tiếp.
...
- **Giảm thiểu:** Duy trì hoạt động khởi động không có cổng phía sau `scripts/start-dev.mjs`, hiện sử dụng tuyến `*.bitsocial.localhost:1355` trong phạm vi nhánh bên ngoài trường hợp chính tắc và quay trở lại tuyến trong phạm vi nhánh khi tên `bitsocial.localhost` trần đã được sử dụng.
- **Trạng thái:** đã xác nhận
