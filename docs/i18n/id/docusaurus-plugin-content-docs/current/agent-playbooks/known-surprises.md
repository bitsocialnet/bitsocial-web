...
- **Mitigasi:** Gunakan `git commit --no-verify -m "message"` untuk komitmen yang dibuat agen. Manusia masih dapat menggunakan `corepack yarn commit` atau `corepack yarn exec cz`.
- **Status:** dikonfirmasi
...
- **Apa yang mengejutkan:** Mesin masih memiliki instalasi Yarn klasik global di `PATH`, jadi menjalankan `yarn` biasa dapat diselesaikan ke v1, bukan versi Yarn 4 yang disematkan.
...
- **Mitigasi:** Gunakan `corepack yarn ...` untuk perintah shell, atau jalankan `corepack enable` terlebih dahulu sehingga `yarn` memutuskan untuk menggunakan versi Yarn 4 yang disematkan.
- **Status:** dikonfirmasi
...
- **Tanggal:** 30-03-2026
- **Diamati oleh:** Codex
- **Konteks:** Memulai `yarn start` di satu pohon kerja Web Bitsocial sementara pohon kerja lain sudah melayani melalui Portless
...
- **Mitigasi:** Pertahankan startup Portless di belakang `scripts/start-dev.mjs`, yang sekarang menggunakan rute cakupan cabang `*.bitsocial.localhost:1355` di luar kasus kanonis dan kembali ke rute cakupan cabang ketika nama kosong `bitsocial.localhost` sudah digunakan.
- **Status:** dikonfirmasi
...
- **Tanggal:** 30-03-2026
- **Diamati oleh:** Codex
- **Konteks:** Berjalan `yarn start` bersama repo dan agen lokal lainnya
- **Yang mengejutkan:** Perintah root dev menjalankan ruang kerja dokumen dengan `docusaurus start --port 3001`, sehingga seluruh sesi dev gagal setiap kali proses lain sudah dimiliki `3001`, meskipun aplikasi utama sudah menggunakan Portless.
- **Dampak:** `yarn start` dapat menghentikan proses web segera setelah boot, sehingga mengganggu pekerjaan lokal yang tidak terkait karena tabrakan port dokumen.
- **Mitigasi:** Pertahankan permulaan dokumen di belakang `yarn start:docs`, yang kini menggunakan Portless plus `scripts/start-docs.mjs` untuk tetap menggunakan port bebas yang dimasukkan atau kembali ke port berikutnya yang tersedia saat dijalankan secara langsung.
- **Status:** dikonfirmasi
